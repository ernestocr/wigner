...
%DIF DEL template-old.tex   Thu Dec 21 23:57:56 2023
%DIF ADD template.tex       Thu Dec 21 23:47:40 2023
...
\DIFdelbegin \DIFdel{commutative (satisfying }\DIFdelend \DIFaddbegin \textit{\DIFadd{commutative}} \DIFadd{curves $\{\Gamma =\left( \alpha(\tau),\beta(\tau)\right),
\; \tau \in \mathbb{F}_{p^{n}}\}$ \mbox{%DIFAUXCMD
...
values}\DIFaddend . In addition, there are $p^{n}-1$ parallel curves
\DIFaddbegin \DIFadd{$\{\Gamma_{\kappa}^{\lambda}$, $\kappa \in \mathbb{F}_{p^{n}}\}$ }\DIFaddend for each curve
passing through the origin.  The whole bundle of $p^{n}$ parallel curves (called
a striation) covers the DPS\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and at every phase-space point $p^{n}+1$ curves
...
\begin{equation}
  \hat{w}\left( 0,0\right)
  = \DIFdelbegin \DIFdel{\frac{1}{p^{n}} }\DIFdelend \sum_{\lambda} |\psi_{0}^{\lambda}\rangle
...
$D\left(\alpha_{\lambda}(\tau),\beta_{\lambda}(\tau)\right)$. \DIFaddbegin
...
\begin{equation}
  \hat{w}(\alpha,\beta)
  = \DIFdelbegin \DIFdel{\frac{1}{p^{n}} }\DIFdelend
...
$, which labels the elements of a stabilizer $\hat{D}^{\lambda}(\tau)$, with \DIFdelbegin \DIFdel{a character $\chi(\kappa \tau)$ }\DIFdelend \DIFaddbegin \DIFadd{the
eigenvalues $\chi(\kappa\tau)$, $\kappa \in F_{p^{n}}$ }\DIFaddend that fixes a particular
stabilizer state \DIFdelbegin \DIFdel{, $\hat{D}^{\lambda}(\tau)|\psi_{\kappa
}^{\lambda}\rangle = \chi(\kappa \tau )|\psi_{\kappa}^{\lambda }\rangle$, that
is, }\DIFdelend \DIFaddbegin \DIFadd{according to (\ref{states}),
...
\DIFaddend \begin{equation}
  ...
  |\psi\DIFdelbegin \DIFdel{_{\kappa }^{\lambda }}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa}^{\lambda}}\DIFaddend \rangle
...
\DIFdelbegin \DIFdel{kernels }\DIFdelend \DIFaddbegin \DIFadd{kernel
}\DIFaddend $\hat{w}\left(0,0\right)$ and thus,
$\hat{w}\left(\alpha,\beta\right)$ are factorized in an almost self-dual basis
for any partition,
...
corresponding to \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{some other }\DIFaddend partition $\pi^{\prime}$.  \DIFaddbegin \DIFadd{Actually, the
...
where $\chi(\alpha)=(-1)^{\tr(\alpha)}$. For instance, for all positive signs
we have \cite{klimov06},
...
$|\psi_{\kappa}^{\lambda}(\pi)\rangle$ from \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an  arrangement $\pi$ in the }\DIFaddend (core)
partition \DIFdelbegin \DIFdel{$\pi$ in
the arrangement }\DIFdelend $\pi_{0}$ has the form \DIFaddbegin \DIFadd{(see Appendix \ref{appA})}\DIFaddend ,
...
along $\Gamma^{\lambda}(\pi)$ and $\Phi_{\gamma,\delta}^{\pi_{0}}(\tau)$ are
...
form (\ref{T2}) if the phases $\Phi_{\gamma,\delta
}^{\pi_{0}}(\tau )$ and $\Phi_{\gamma,\delta}^{\pi}(\tau )$ are
the same along the curve $\Gamma^{\lambda}(\pi)$. Such curves will be referred
as to \textit{abelian curves}. Obviously, all the curves that constitute a
given partition are abelian. However, there are abelian curves outside of this
partition, leading to a DWF of the corresponding stabilizer states being
delta-functions\DIFaddbegin \DIFadd{, }
...
curves (\ref{rc}) are abelian under the positive sign selection (\ref{Phib}),
if the following relation between parameters $\phi_{0}$ and $\phi$ is
fulfilled:
