...
%DIF DEL template-old.tex   Wed Dec 20 20:56:24 2023
%DIF ADD template.tex       Wed Dec 20 20:55:35 2023
...
%DIF 76c77
%DIF < \Title{Tomographic universality of the discrete Wigner function}
%DIF -------
...
%DIF 79c80
%DIF < \TitleCitation{Tomographic universality of the discrete Wigner function}
%DIF -------
...
%DIF 130-136c131-136
%DIF < 	kernel, whose explicit form depends on a particular partition of the
%DIF < 	discrete phase-space. Nonetheless, it turns out that the DWF for some
%DIF < 	specific stabilizers, not included in the set used for the construction of
%DIF < 	the Wigner map, takes on the form of a delta-function. This implies that the
%DIF < 	possibility of classical simulations of Pauli measurements in a given
%DIF < 	stabilizer state for qubit systems is closely tied to the experimental
%DIF < 	setup.
%DIF -------
...
% Keywords
%DIF 140-141c140-143
%DIF < \keyword{discrete Wigner function; quantum tomography; stabilizer formalism;
%DIF < Mutually Unbiased Bases.} 
%DIF -------
\keyword{ %DIF > 
...
%DIF < 
%DIF < The template details the sections that can be used in a manuscript. Note that the order and names of article sections may differ from the requirements of the journal (e.g., the positioning of the Materials and Methods section). Please check the instructions on the authors' page of the journal to verify the correct order and names. For any questions, please contact the editorial office of the journal or support@mdpi.com. For LaTeX-related questions please contact latex@mdpi.com.%\endnote{This is an endnote.} % To use endnotes, please un-comment \printendnotes below (before References). Only journal Laws uses \footnote.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  The order of the section titles is different for some journals. Please refer to the "Instructions for Authors” on the journal homepage.
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{non-intersecting }\DIFdelend \DIFaddbegin \DIFadd{parallel }\DIFaddend lines correspond to \DIFaddbegin \DIFadd{different components of the same basis, while
(single) intersecting lines are related to states belonging to the }\DIFaddend mutually
...
\DIFaddbegin \DIFadd{The structure of this paper is as follows: In Sec. 2 we recall the basic
...
\begin{equation}
  ...
  \quad
  \kappa = \sum_{i=1}^{n} k_{i} \, \theta_{i} \in \mathbb{F}_{p^{n}},
  \label{basis}
\end{equation}
...
$\tr(\theta_{i}\,\tilde{\theta}_{j})=\delta_{ij}$], here \DIFdelbegin \DIFdel{$\tr(\alpha) = \alpha
+\alpha ^{2}+\ldots +\alpha ^{p^{n-1}}$}\DIFdelend \DIFaddbegin \DIFadd{$\tr(\alpha) = \alpha
+\alpha^{2}+\ldots + \alpha ^{p^{n-1}}$}\DIFaddend , for $\alpha \in \mathbb{F}_{p^{n}}$
\cite{FF}.

In even dimensions there always exists a self-dual basis, i.e. a basis such that
\DIFdelbegin \DIFdel{$\tr(\theta _{i}\,\theta _{j})=\delta _{ij}$}\DIFdelend \DIFaddbegin \DIFadd{$\tr(\theta_{i}\,\theta_{j})=\delta_{ij}$}\DIFaddend , whereas in odd dimensions there are
\textit{almost} self-dual bases, such that
\DIFdelbegin \DIFdel{$\tr(\theta _{i}\,\theta
_{j})=q_{j}\delta _{ij}$}\DIFdelend \DIFaddbegin \DIFadd{$\tr(\theta_{i}\,\theta_{j})=q_{j}\delta_{ij}$}\DIFaddend , where $q_{j}$ is equal to $1$
with one possible exception, e.g. \DIFdelbegin \DIFdel{$q_{j}=(q-1)\delta_{jn}+1$ for some $q \in \mathbb Z_p$}\DIFdelend \DIFaddbegin \DIFadd{$q_{j} = (q-1)\delta_{jn}+1$ for some $q \in
\mathbb{Z}_p$}\DIFaddend .
...
\begin{equation}
  \chi\left( \alpha \beta \right)
  %DIF > = \Pi_{j=1}^{n} \omega{\left( \alpha_{j}\beta_{j}q_{j}\right)},
  = \Pi_{j=1}^{n} \omega\DIFdelbegin %DIFDELCMD < \left( %%%
\DIFdel{\alpha_{j}\beta _{j}q_{j}}%DIFDELCMD < \right)%%%
...
where \DIFdelbegin \DIFdel{$\Phi_{\alpha ,\beta }\left( \tau \right) \Phi _{\alpha ,\beta }^{\ast
}\left( \tau \right) =1$ }\DIFdelend \DIFaddbegin \DIFadd{$\Phi_{\alpha ,\beta }\left( \tau \right) \Phi_{\alpha ,\beta }^{\ast
}\left( \tau \right) = 1$ }\DIFaddend is a phase factor. The operators (\ref{D}) from each
commuting set \DIFdelbegin \DIFdel{$\lambda =\{\alpha,\beta\}$ form an Abelian }\DIFdelend \DIFaddbegin \DIFadd{$\lambda =\{\alpha ,\beta\}$ form an abelian }\DIFaddend group,
...
\begin{equation}
  ...
  = \hat{D}\left(
    ...
    \beta_{\lambda}(\tau + \tau\DIFdelbegin \DIFdel{^{\prime }}\DIFdelend \DIFaddbegin \DIFadd{^{\prime}}\DIFaddend )
  \right),
  \label{DD}
\end{equation}

as a consequence of the relation \DIFdelbegin \DIFdel{$\hat{Z}_{\tau }\hat{Z}_{\tau ^{\prime }}=
\hat{Z}_{\tau +\tau ^{\prime }}$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{Z}_{\tau }\hat{Z}_{\tau ^{\prime}} =
\hat{Z}_{\tau +\tau^{\prime}}$}\DIFaddend . This implies that the phases \DIFdelbegin \DIFdel{$\Phi
_{\alpha,\beta }\left( \tau \right) $, where $(\alpha ,\beta )\in \lambda $, $|\Phi _{\alpha ,\beta }\left( \tau \right) |=1$,
$\Phi_{\alpha,\beta}\left(0\right) = 1$ }\DIFdelend \DIFaddbegin \DIFadd{$\Phi_{\alpha,\beta
}\left( \tau \right) $, where $(\alpha ,\beta )\in \lambda$, $\Phi_{\alpha
,\beta }\left( 0\right) = 1$ }\DIFaddend satisfy the equation,

\begin{equation}
  \Phi\DIFdelbegin \DIFdel{_{\alpha,\beta }}\DIFdelend \DIFaddbegin \DIFadd{_{\alpha,\beta}}\DIFaddend \left( \tau \right)
  \Phi_{\alpha,\beta }\left(\tau^{\prime}\right)
  = \Phi_{\alpha,\beta }\left( \tau +\tau\DIFdelbegin \DIFdel{^{\prime }}\DIFdelend \DIFaddbegin \DIFadd{^{\prime}}\DIFaddend \right)
...
to a partition of the DPS into non-intersecting (except \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the origin),
non-degenerate (containing $p^{n}$ different points including the origin)
\DIFdelbegin \DIFdel{commutative (satisfying }\DIFdelend \DIFaddbegin \textit{\DIFadd{commutative}} \DIFadd{curves $\{\Gamma =\left( \alpha (\tau ),\beta
(\tau)\right), \; \tau \in \mathbb{F}_{p^{n}}\}$ \mbox{%DIFAUXCMD
\cite{GS2,JPA09}}\hskip0pt%DIFAUXCMD
. In other
words, the points of every commutative curve $\Gamma$ label a set of mutually
commutative monomials,
}

\begin{equation}
  \DIFadd{\{\hat{Z}_{\alpha (\tau )}\hat{X}_{\beta (\tau )}, \;
  }[\DIFadd{\hat{Z}_{\alpha(\tau)} \hat{X}_{\beta (\tau )},
  \hat{Z}_{\alpha (\tau ^{\prime })}\hat{X}_{\beta(\tau^{\prime })}}]
  \DIFadd{= 0, \; \left( \alpha (\tau ),\beta (\tau )\right) \in \Gamma \},
}\end{equation}

\DIFadd{such that }\DIFaddend the condition (\ref{CC}) \DIFdelbegin \DIFdel{) curves $\{\Gamma
=\left( \alpha (\tau ),\beta (\tau )\right) ,\;\tau \in \mathbb{F}_{p^{n}}\}$
\mbox{%DIFAUXCMD
\cite{GS2,JPA09}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{is satisfied. There exist $p^{n}+1$
...
values}\DIFaddend . In addition, there are $p^{n}-1$ parallel curves \DIFaddbegin \DIFadd{$\{\Gamma _{\kappa
}^{\lambda}$, $\kappa \in \mathbb{F}_{p^{n}}\}$ }\DIFaddend for each curve passing through
the origin.  The whole bundle of $p^{n}$ parallel curves (called a striation)
covers the DPS\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and at every phase-space point $p^{n}+1$ curves (i.e.  one curve
from each striation) intersect. It is worth stressing that only the monomials,
...
\begin{align}
  ...
  &= \DIFdelbegin \DIFdel{\mathrm{Tr}}\DIFdelend \DIFaddbegin \DIFadd{\Tr }\DIFaddend \left[ \hat{f}\hat{w}\left(\alpha ,\beta \right) \right],
  \label{map} \\
  \hat{f}
  &= p^{-n}\sum_{\alpha, \beta } W_{f}(\alpha,\beta) \hat{w}(\alpha,\beta),
\end{align}

where the Hermitian mapping kernel \DIFdelbegin \DIFdel{$\hat{w}\left(\alpha,\beta\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{w}\left( \alpha ,\beta \right) =
\hat{w}^{\dagger }(\alpha ,\beta )$ }\DIFaddend is covariant under action of the discrete
...
\begin{eqnarray}
  ...
  \hat{w}\left( 0,0\right) \hat{D}^{\dagger }\left( \alpha ,\beta \right) \DIFdelbegin \DIFdel{= }%DIFDELCMD < \hat{w}%%%
\DIFdel{^{\dagger }(\alpha }\DIFdelend ,
  \DIFdelbegin \DIFdel{\beta ),
  }\DIFdelend \label{Ds} \\
  \hat{w}\left( 0,0\right)
  &=&\DIFdelbegin \DIFdel{p^{-n} }\DIFdelend \sum\DIFdelbegin \DIFdel{_{\gamma,\delta} }\DIFdelend \DIFaddbegin \DIFadd{_{\gamma ,\delta }}\DIFaddend \hat{D}(\gamma ,\delta ),
  \label{Dw0}
\end{eqnarray}
...
\DIFaddend The kernel (\ref{Ds}) is normalized according to,
...
\begin{equation}
  ...
  = p^{n} \delta_{\alpha \alpha^{\prime}} \delta\DIFdelbegin \DIFdel{_{\beta \beta^{\prime}}}\DIFdelend \DIFaddbegin \DIFadd{_{\beta\beta^{\prime}}}\DIFaddend .
\end{equation}

It is convenient to recast Eq. (\ref{Dw0}) as a sum of stabilizers corresponding
to a given \DIFaddbegin \DIFadd{phase-space }\DIFaddend partition,
...
\begin{equation}
  \hat{w}\left( 0,0\right)
  = \DIFdelbegin \DIFdel{\frac{1}{p^{n}} }\DIFdelend \sum\DIFdelbegin \DIFdel{_{\lambda}
  }\DIFdelend \DIFaddbegin \DIFadd{_{\lambda } }\DIFaddend |\psi _{0}\DIFdelbegin \DIFdel{^{\lambda}}\DIFdelend \DIFaddbegin \DIFadd{^{\lambda }}\DIFaddend \rangle
  \langle \psi _{0}^{\lambda }| - \hat{I},
  \label{w0}
\end{equation}

where the index $\lambda$, labels the stabilizers/projectors \DIFaddbegin \DIFadd{(\ref{states})
}\DIFaddend within this partition. Geometrically, Eqs. (\ref{Dw00}) and (\ref{w0}) represent
a summation along the curves that pass through the origin, i.e. of the form
(\ref{curve1}). In other words, each point \DIFdelbegin \DIFdel{$(\alpha,\beta)$ }\DIFdelend \DIFaddbegin \DIFadd{$(\alpha, \beta)$ }\DIFaddend of the DPS is
assigned a specific phase \DIFdelbegin \DIFdel{$\Phi_{\alpha,\beta } \left( \tau \right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Phi_{\alpha, \beta}\left( \tau \right)$ }\DIFaddend according to
the unit commutative curve $\Gamma^{\lambda}$ that passes through that point and
corresponding to the displacement operator
\DIFdelbegin \DIFdel{$D\left(\alpha_{\lambda }(\tau
),\beta_{\lambda }(\tau)\right)$.  }\DIFdelend \DIFaddbegin \DIFadd{$D\left(\alpha_{\lambda}(\tau),\beta_{\lambda}(\tau)\right)$.  In
...
\DIFaddbegin \begin{figure}[ht]
  ...
  \caption{
    \DIFaddFL{Unit commutative curves along with the distribution of the respective phases
...
\DIFaddend By shifting $\hat{w}\left(0,0\right)$ through the action of the displacement
operators (\ref{Ds}) to an arbitrary point $(\alpha,\beta )$ one obtains

\begin{equation}
  \hat{w}(\alpha, \beta)
  = \DIFdelbegin \DIFdel{\frac{1}{p^{n}} }\DIFdelend \sum\DIFdelbegin \DIFdel{_{\lambda (\alpha,\beta),\kappa(\alpha ,\beta )}
  }\DIFdelend \DIFaddbegin \DIFadd{_{\lambda(\alpha,\beta),\kappa(\alpha,\beta)}
  }\DIFaddend |\psi \DIFdelbegin \DIFdel{_{\kappa}^{\lambda}}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa }^{\lambda }}\DIFaddend \rangle \langle \psi \DIFdelbegin \DIFdel{_{\kappa}}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa }}\DIFaddend ^{\lambda }|
  - \hat{I},
  \label{wab}
\end{equation}
...
The Wigner function of any stabilizer state \DIFdelbegin \DIFdel{$|\psi_{\kappa }^{\lambda}\rangle$
}\DIFdelend \DIFaddbegin \DIFadd{$|\psi _{\kappa }^{\lambda }\rangle
$ }\DIFaddend within a partition used for the construction of \DIFdelbegin \DIFdel{$\hat{w}\left(0,0\right)$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{w}\left( 0,0\right) $}\DIFaddend ,
is a convolution of characters computed at the points of the unit commutative
curve \DIFdelbegin \DIFdel{$\Gamma^{\lambda} = \left(\gamma_{\lambda}\left(\tau \right),
\delta_{\lambda}(\tau)\right)$}\DIFdelend \DIFaddbegin \DIFadd{$\Gamma ^{\lambda }=\left( \gamma _{\lambda }\left( \tau \right) ,\delta
_{\lambda }(\tau )\right) $}\DIFaddend , which labels the elements of a stabilizer
\DIFdelbegin \DIFdel{$\hat{D}^{\lambda}(\tau)$, with a character $\chi(\kappa \tau)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{D}^{\lambda }(\tau )$, with the eigenvalues $\chi (\kappa \tau )$, $\kappa
\in F_{p^{n}}$ }\DIFaddend that fixes a particular stabilizer state \DIFdelbegin \DIFdel{, $\hat{D}^{\lambda}(\tau)|\psi_{\kappa
}^{\lambda}\rangle = \chi(\kappa \tau )|\psi_{\kappa}^{\lambda }\rangle$, that
is, }\DIFdelend \DIFaddbegin \DIFadd{according to
(\ref{states}),
}\DIFaddend 

\DIFaddbegin \begin{equation}
  \DIFadd{\langle \psi_{\kappa }^{\lambda }|\hat{w}\left( \alpha ,\beta \right)
...
\DIFadd{as it immediately follows from the spectral decomposition (\ref{Dexp}) and Eq.
(\ref{wabD}).
}

\DIFadd{On the other hand, it immediately follows from the decomposition (\ref{wab}) and
...
\DIFaddend \begin{equation}
  ...
  |\psi_{\kappa }^{\lambda }\rangle
  = \sum\DIFdelbegin \DIFdel{_{\tau } \chi}%DIFDELCMD < \left(
%DIFDELCMD <     %%%
\DIFdel{\gamma_{\lambda}(\tau) \beta + \alpha }\DIFdelend \DIFaddbegin \DIFadd{_{\lambda^{\prime}(\alpha,\beta),\kappa^{\prime }(\alpha,\beta)}
  |}\langle
    \DIFadd{\psi_{\kappa^{\prime}}^{\lambda^{\prime }}|\psi_{\kappa}^{\lambda}
  }\rangle\DIFadd{|^{2}-1
  = \sum_{\lambda^{\prime}(\alpha,\beta),\kappa^{\prime}(\alpha,\beta)}
  }\DIFaddend \delta\DIFdelbegin \DIFdel{_{\lambda}(\tau)
  }%DIFDELCMD < \right)
%DIFDELCMD <   %%%
\DIFdel{\chi(\kappa \tau).
  }%DIFDELCMD < \label{T1}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{_{\lambda,\lambda^{\prime}}\delta_{\kappa,\kappa^{\prime}},
}\DIFaddend \end{equation}

\DIFdelbegin \DIFdel{The former expression can be reduced to }\DIFdelend \DIFaddbegin \DIFadd{which is just }\DIFaddend a delta-function along the curve \DIFdelbegin \DIFdel{$\Gamma_{\kappa}^{\lambda} = \{(\alpha_{\lambda}(\tau,\kappa),
\beta_{\lambda}(\tau,\kappa)), \, \tau \in \mathbb{F}_{p^{n}}\},$ which is
}\DIFdelend \DIFaddbegin \DIFadd{$\Gamma_{\kappa}^{\lambda} =
\{(\alpha_{\lambda }(\tau,\kappa),\beta_{\lambda}(\tau,\kappa)),\,\tau \in
\mathbb F_{p^{n}}\}$ }\DIFaddend associated with the state \DIFdelbegin \DIFdel{$|\psi_{\kappa}^{\lambda}\rangle$}\DIFdelend \DIFaddbegin \DIFadd{$|\psi_{\kappa }^{\lambda
}\rangle$}\DIFaddend :

\begin{equation}
  \langle \psi\DIFdelbegin \DIFdel{_{\kappa }^{\lambda }
  }\DIFdelend \DIFaddbegin \DIFadd{_{\kappa}^{\lambda}
  }\DIFaddend |\hat{w}\left( \alpha,\beta \right)
  |\psi\DIFdelbegin \DIFdel{_{\kappa}}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa }}\DIFaddend ^{\lambda}
  \rangle = \sum_{\tau} \delta\DIFdelbegin \DIFdel{_{\alpha_{\lambda}(\tau,\kappa), \beta_{\lambda}(\tau,\kappa )}}\DIFdelend \DIFaddbegin \DIFadd{_{\alpha_{\lambda}(\tau,\kappa),
  \beta_{\lambda}(\tau ,\kappa )}}\DIFaddend .
  \label{T2}
\end{equation}
...
\DIFdelbegin \begin{displaymath}
  \DIFdel{\hat{w}\left(\alpha,\beta\right)
...
\begin{equation*}
  ...
  = \DIFdelbegin \DIFdel{\frac{1}{p} }\DIFdelend \DIFaddbegin \DIFadd{p^{-1} }\DIFaddend \sum_{k,l} \omega\DIFdelbegin \DIFdel{(\beta_{j} k - \alpha_{j} l -2^{-1} k l)
  }\DIFdelend \DIFaddbegin \DIFadd{^{(\beta_{j} k - \alpha_{j} l - 2^{-1} k l)}
  }\DIFaddend \hat{Z}^{l} \,\hat{X}^{k}.
\end{equation*}

This implies that the mapping kernel \DIFdelbegin \DIFdel{$\hat{w}\left(\alpha,\beta\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{w}\left( \alpha ,\beta \right)$ }\DIFaddend does
not depend on the particular partition \DIFaddbegin \DIFadd{of the DPS in the case of odd local
dimension}\DIFaddend . Such a statement may not be immediately obvious from the
...
for any stabilizer state \DIFdelbegin \DIFdel{$|\psi_{\kappa}^{\lambda}(\pi)\rangle$ }\DIFdelend \DIFaddbegin \DIFadd{$|\psi_{\kappa }^{\lambda }(\pi )\rangle $ }\DIFaddend belonging to
an arbitrary phase-space partition \DIFdelbegin \DIFdel{$\pi$}\DIFdelend \DIFaddbegin \DIFadd{$\pi $}\DIFaddend , even if they are unrelated to the one
employed for the construction of the Wigner map\DIFdelbegin \DIFdel{.}\DIFdelend , i.e.,
...
where \DIFdelbegin \DIFdel{$\hat{w}\left(\alpha,\beta \, | \, \pi^{\prime}\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{w}\left( \alpha ,\beta \,|\,\pi ^{\prime }\right) $ }\DIFaddend is a Wigner
kernel corresponding to \DIFdelbegin \DIFdel{a partition $\pi^{\prime}$.  }\DIFdelend \DIFaddbegin \DIFadd{some other partition $\pi ^{\prime }$.  Actually, the
...
\begin{equation}
  \Phi_{\alpha ,\beta }(\theta _{k})=\pm \DIFdelbegin \DIFdel{\sqrt{\chi \left( \alpha _{\lambda
}(\theta _{k})\beta _{\lambda }(\theta _{k})\right) }}\DIFdelend \DIFaddbegin \DIFadd{\sqrt{\chi \left( \alpha_{\lambda
  }(\theta_{k})\beta_{\lambda }(\theta _{k})\right) }}\DIFaddend ,
  \label{Phib}
\end{equation}
...
\begin{equation}
  \Phi\DIFdelbegin \DIFdel{_{\alpha,\beta }}\DIFdelend \DIFaddbegin \DIFadd{_{\alpha,\beta}}\DIFaddend \left(
  ...
  \right)
  \prod\limits_{k=1}^{n}c_{\kappa }\DIFdelbegin \DIFdel{^{\alpha,\beta}}\DIFdelend \DIFaddbegin \DIFadd{^{\alpha ,\beta }}\DIFaddend ,
  \label{Phi G}
\end{equation}

where \DIFdelbegin \DIFdel{$c_{\kappa}^{\alpha,\beta} =
\sqrt{\chi\left(\alpha_{\lambda}(t_{k}\theta_{k})
\beta_{\lambda}\left(\alpha_{k} \theta_{k}\right)\right)}$}\DIFdelend \DIFaddbegin \DIFadd{$c_{\kappa }^{\alpha ,\beta }=\sqrt{\chi \left( \alpha _{\lambda
}(t_{k}\theta _{k})\beta _{\lambda }\left( \alpha _{k}\theta _{k}\right) \right)
}$}\DIFaddend . The non-factorized form of the phase \DIFdelbegin \DIFdel{$\Phi_{\alpha,\beta}$ }\DIFdelend \DIFaddbegin \DIFadd{$\Phi _{\alpha ,\beta }$ }\DIFaddend implies that
the map \DIFdelbegin \DIFdel{$\hat{w}(\alpha,\beta)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{w}(\alpha ,\beta )$ }\DIFaddend essentially depends on the choice of a
complete set of disjoint stabilizers. In other words, it depends on the
partition of the DPS into sets of commutative curves \DIFdelbegin \DIFdel{$\Gamma^{\lambda}$}\DIFdelend \DIFaddbegin \DIFadd{$\Gamma ^{\lambda }$}\DIFaddend .

In general, the Wigner function of a stabilizer state \DIFdelbegin \DIFdel{$|\psi_{\kappa}^{\lambda}(\pi)\rangle$ from a }\DIFdelend \DIFaddbegin \DIFadd{$|\psi_{\kappa }^{\lambda
}(\pi )\rangle $ from an  arrangement $\pi $ in the }\DIFaddend (core) partition \DIFdelbegin \DIFdel{$\pi$ in
the arrangement }\DIFdelend $\pi_{0}$
has the form \DIFaddbegin \DIFadd{(see Appendix \ref{appA})}\DIFaddend ,

\begin{equation}
  \langle \psi \DIFdelbegin \DIFdel{_{\kappa}^{\lambda}}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa }^{\lambda }}\DIFaddend (\pi )
  |\hat{w}\left( \alpha ,\beta\,|\,\pi _{0}\right)
  |\psi \DIFdelbegin \DIFdel{_{\kappa}^{\lambda}}\DIFdelend \DIFaddbegin \DIFadd{_{\kappa }^{\lambda }}\DIFaddend (\pi )\rangle
  = \sum_{\tau} \chi\left(
    \gamma\DIFdelbegin \DIFdel{_{\lambda} }\DIFdelend \DIFaddbegin \DIFadd{_{\lambda }}\DIFaddend (\tau )\beta +\alpha \delta \DIFdelbegin \DIFdel{_{\lambda}}\DIFdelend \DIFaddbegin \DIFadd{_{\lambda }}\DIFaddend (\tau)
  \right)
  \chi(\kappa \tau ) \Phi\DIFdelbegin \DIFdel{_{\gamma,\delta}}\DIFdelend \DIFaddbegin \DIFadd{_{\gamma, \delta}}\DIFaddend ^{\pi_{0}}(\tau)
  \Phi\DIFdelbegin \DIFdel{_{\gamma,\delta }}\DIFdelend \DIFaddbegin \DIFadd{_{\gamma,\delta}}\DIFaddend ^{\pi \ast}(\tau),
  \label{WF qubit}
\end{equation}

where \DIFdelbegin \DIFdel{$\left(\gamma_{\lambda}(\tau),\delta_{\lambda}(\tau)\right) \in
\Gamma^{\lambda}(\pi)$, $\Phi_{\gamma,\delta }^{\pi}(\tau)$ }\DIFdelend \DIFaddbegin \DIFadd{$\left( \gamma_{\lambda }(\tau ),\delta_{\lambda }(\tau )\right) \in
\Gamma ^{\lambda }(\pi )$, $\Phi_{\gamma ,\delta }^{\pi }(\tau )$ }\DIFaddend are the phases
along \DIFdelbegin \DIFdel{$\Gamma^{\lambda}(\pi)$ and $\Phi_{\gamma,\delta}^{\pi_{0}}(\tau)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Gamma ^{\lambda }(\pi )$ and $\Phi _{\gamma ,\delta }^{\pi_{0}}(\tau )$
}\DIFaddend are the phases assigned to each point \DIFdelbegin \DIFdel{$\left(\gamma_{\lambda}(\tau),\delta_{\lambda}(\tau )\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\left( \gamma_{\lambda }(\tau
),\delta_{\lambda }(\tau )\right) $ }\DIFaddend of the DPS according to the partition \DIFdelbegin \DIFdel{$\pi_{0}$}\DIFdelend \DIFaddbegin \DIFadd{$\pi
_{0}$}\DIFaddend .  Therefore, Eq. (\ref{WF qubit}) can be reduced to the form (\ref{T2}) if
the phases \DIFdelbegin \DIFdel{$\Phi_{\gamma,\delta}^{\pi_{0}}(\tau)$ and $\Phi_{\gamma,\delta }^{\pi}(\tau)$
}\DIFdelend \DIFaddbegin \DIFadd{$\Phi_{\gamma,\delta }^{\pi_{0}}(\tau )$ and $\Phi _{\gamma ,\delta
}^{\pi }(\tau )$ }\DIFaddend are the same along the curve \DIFdelbegin \DIFdel{$\Gamma^{\lambda}(\pi)$}\DIFdelend \DIFaddbegin \DIFadd{$\Gamma^{\lambda }(\pi )$}\DIFaddend . Such
...
states being delta-functions\DIFaddbegin \DIFadd{,
}

\begin{equation}
  \DIFadd{\langle \psi_{\kappa }^{\lambda}
...
\DIFadd{where $\Gamma_{\kappa }^{\lambda}(\pi)$ is a curve associated with the state
$|\psi_{\kappa }^{\lambda }(\pi)\rangle$ in the partition $\pi$}\DIFaddend . It is worth
...
\DIFadd{The density matrix of a stabilizer state corresponding to an abelian curve not
belonging to the core partition, aquires a simple form:
}

\begin{equation}
  \DIFadd{|\psi_{\kappa }^{\lambda}(\pi)\rangle
...
\DIFadd{i.e., it is an equally weighted sum of the Wigner kernels in the core partition
...
\DIFaddend In general, searching for abelian curves that do not belong to a given partition
...
displacement operators along such curves from a partition \DIFdelbegin \DIFdel{$\pi$}\DIFdelend \DIFaddbegin \DIFadd{$\pi $}\DIFaddend , as for those
of the core partition \DIFdelbegin \DIFdel{$\pi_{0}$}\DIFdelend \DIFaddbegin \DIFadd{$\pi _{0}$}\DIFaddend . In what follows we give examples of abelian
...
\begin{equation}
  \DIFaddbegin \left[\DIFaddend \Phi\DIFaddbegin \right]\DIFadd{_{\alpha ,\beta }
    ...
    \DIFdelbegin %DIFDELCMD < \begin{array}{ccccccccc}
%DIFDELCMD <     \alpha \backslash \beta & 0 & \sigma & \sigma ^{2} & \sigma ^{3} & \sigma
%DIFDELCMD <     ^{4} & \sigma ^{5} & \sigma ^{6} & \sigma ^{7} \\ 
%DIFDELCMD <     0 & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} \\ 
%DIFDELCMD <     \sigma & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-1} & \multicolumn{1}{r}{
%DIFDELCMD <     -i} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-i} & \multicolumn{1}{r}{i}
%DIFDELCMD <     & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-1} \\ 
%DIFDELCMD <     \sigma ^{2} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-i} & 
%DIFDELCMD <     \multicolumn{1}{r}{-1} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-i} & 
%DIFDELCMD <     \multicolumn{1}{r}{i} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-1} \\ 
%DIFDELCMD <     \sigma ^{3} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & 
%DIFDELCMD <     \multicolumn{1}{r}{i} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{i} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} \\ 
%DIFDELCMD <     \sigma ^{4} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-i} & 
%DIFDELCMD <     \multicolumn{1}{r}{-i} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-1} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-1} \\ 
%DIFDELCMD <     \sigma ^{5} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} & 
%DIFDELCMD <     \multicolumn{1}{r}{i} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & 
%DIFDELCMD <     \multicolumn{1}{r}{i} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} \\ 
%DIFDELCMD <     \sigma ^{6} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} & 
%DIFDELCMD <     \multicolumn{1}{r}{1} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{i} \\ 
%DIFDELCMD <     \sigma ^{7} & \multicolumn{1}{r}{1} & \multicolumn{1}{r}{-1} & 
%DIFDELCMD <     \multicolumn{1}{r}{-1} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-1} & 
%DIFDELCMD <     \multicolumn{1}{r}{i} & \multicolumn{1}{r}{i} & \multicolumn{1}{r}{-i}
%DIFDELCMD <   \end{array}%%%
...
\DIFdelbegin \DIFdel{$
\tr(\alpha \sigma_{1}) = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\tr(\alpha \sigma_{1}) = 0$ }\DIFaddend and $\tr(\beta \sigma_{2}) = 0$, where
...
curves (\ref{rc}) are abelian under the positive sign \DIFdelbegin \DIFdel{election }\DIFdelend \DIFaddbegin \DIFadd{selection }\DIFaddend (\ref{Phib}), if
...
\begin{enumerate}
  ...
  \DIFaddend \item if $\phi = \sigma^{3}, \sigma^{5}, \sigma^{6}$, then $\phi_{0}=0,1$,
\DIFaddbegin 

  \DIFaddend \item if $\phi = 1$, there are no $\phi_{0}$ producing abelian curves\DIFaddbegin \DIFadd{, as can
    be observed in the example of Figure~\ref{fig1} (b)}\DIFaddend .
\end{enumerate}
...
%DIFDELCMD < %%%
\DIFdelend are not abelian, \DIFdelbegin \DIFdel{so that the corresponding DWFs }\DIFdelend \DIFaddbegin \DIFadd{thus the DFWs corresponding to the stabilizer states
$\{|\psi_\kappa^{(\beta; \sigma^2, \sigma^{5})}(0,9,0)\rangle, \kappa \in
\mathbb F_{2^3}\}$ }\DIFaddend are not delta-functions. In Figure~\DIFdelbegin \DIFdel{\ref{fig1} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig2} }\DIFaddend we plot the
Wigner function for the unit stabilizer states corresponding to the curves
(\ref{ac2}) and (\ref{nac}) respectively. \DIFaddbegin \DIFadd{Reconstruction of the stabilizer
...
\begin{figure}[ht]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \subfloat[\centering Abelian curve 
%DIFDELCMD < 	]{{\includegraphics[width=6.5cm]{abelian.eps} }}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[\centering Abelian curve ]{{
    \includegraphics[width=6.5cm]{abelian.eps}
  }}
  \DIFaddendFL \quad 
  \DIFdelbeginFL %DIFDELCMD < \subfloat[\centering Non-abelian curve
%DIFDELCMD < 	]{{\includegraphics[width=6.5cm]{non_abelian.eps} }}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[\centering Non-abelian curve]{{
...
% Optional
\appendixtitles{no} 
%DIF <  Leave argument "no" if all appendix headings stay EMPTY (then no dot is printed after "Appendix A"). If the appendix sections contain a heading then change the argument to "yes".
%DIF >  Leave argument "no" if all appendix headings stay EMPTY (then no dot is
%DIF >  printed after "Appendix A"). If the appendix sections contain a heading then
%DIF >  change the argument to "yes".
\appendixstart\appendix
\DIFaddbegin 

\DIFaddend \section[\appendixname~\thesection]{}
\label{appA}
%DIF < \subsection[\appendixname~\thesubsection]{}

In this Appendix we \DIFaddbegin \DIFadd{derive Eq. (\ref{WF qubit}) of the main text. Taking into
account the expansion inverse to (\ref{Dexp}),
}

\begin{equation}
  \DIFadd{|\psi_{\kappa }^{\lambda}\rangle \langle \psi_{\kappa }^{\lambda}|
  = \sum_{\tau }\chi (\kappa \tau )\hat{D}^{\lambda }(\tau )
}\end{equation}

\DIFadd{and the representation (\ref{wabD}) of the Wigner kernel, one obtains
}

\begin{adjustwidth}{-\extralength}{0cm}
\begin{equation}
  \DIFadd{\langle \psi_{\kappa}^{\lambda}(\pi)
...
\DIFadd{where the sum over $\tau$ denotes a sum over the points of the curve
...
\DIFadd{which after performing a sum over $\gamma$ and $\delta$, is reduced to Eq.
(\ref{WF qubit}).
...
\DIFadd{In this Appendix we }\DIFaddend recall some basic properties of commutative curves in even
local dimensions. Commuting sets constituted by $2^{n}$ different monomials
(stabilizers)
\DIFdelbegin \DIFdel{$\{ \hat{Z}_{\alpha _{\lambda }(\tau )}\hat{X}_{\beta _{\lambda
}(\tau )}, \, \tau \in \mathbb{F}_{2^{n}}\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\hat{Z}_{\alpha_{\lambda}(\tau)}\hat{X}_{\beta_{\lambda}(\tau)}, \, \tau \in
\mathbb{F}_{2^{n}}\}$ }\DIFaddend are labelled by points of a discrete grid belonging to a
...
%DIFDELCMD < 		\alpha = \sigma^{2}\beta + \sigma^{3}\beta^{2} + \sigma^{5}\beta^{4},
%DIFDELCMD < 		\qquad\qquad
%DIFDELCMD < 		& \alpha = \sigma^{6}\beta + \sigma^{3}\beta^{2} +
%DIFDELCMD < 		\sigma^{5}\beta^{4},\\[5pt]
%DIFDELCMD < 		\beta = \sigma^{2}\alpha + \sigma^{3}\alpha^{2} + \sigma^{5}\alpha^{4},
%DIFDELCMD < 		\qquad\qquad
%DIFDELCMD < 		& \beta = \sigma^{6}\alpha^{2} + \sigma^{3}\alpha^{4}, \\[5pt]
%DIFDELCMD < 		\alpha = \beta + \sigma^{6}\beta^{2} + \sigma^{3}\beta^{4},
%DIFDELCMD < 		\qquad\qquad
%DIFDELCMD < 		& \beta = \alpha + \sigma^{3}\alpha^{2} + \sigma^{5}\alpha^{4}, \\[5pt]
%DIFDELCMD < 		\alpha = \sigma^{3}\beta^{2} + \sigma^{5}\beta^{4},
%DIFDELCMD < 		\qquad\qquad
%DIFDELCMD < 		&
%DIFDELCMD < 	  \end{array}%%%
...
\DIFaddbegin \section[\appendixname~\thesection]{}
\label{appC}

\DIFadd{In this Appendix we provide the explicit expressions for the reconstruction of
...
\begin{adjustwidth}{-\extralength}{0cm}
\begin{equation}
  \DIFadd{|\psi _{0}^{(\beta ;1, \sigma^{5})}(0,9,0)\rangle
...
\begin{eqnarray}
  ...
  = \frac{1}{8},%DIF >  \quad \text{for all } \kappa \in \mathbb F_{2^3},
}\end{eqnarray}
\end{adjustwidth}
...
\begin{adjustwidth}{-\extralength}{0cm}
\begin{eqnarray}
  \DIFadd{p_{\kappa }^{\lambda }\left(\sigma^{2}, \sigma^{5}\right)
  }&\DIFadd{=}& \DIFadd{|\langle\psi_{\kappa }^{\lambda }
  ...
  \end{array}\right), }\\
  \DIFadd{\tilde{p}_{\kappa }(\sigma^{2},\sigma^{5})
  }&\DIFadd{=}& \DIFadd{|\langle \tilde\psi_\kappa | \psi_0^{(\beta;\sigma^2,\sigma^5)}(0,9,0)\rangle|^2
  = \frac{1}{8},%DIF >  \quad \text{for all } \kappa \in \mathbb F_{2^3},
}\end{eqnarray}
\end{adjustwidth}

\DIFadd{where the MUBs associated with the partition of the DPS in rays (\ref{rays})
have the form,
}

\begin{eqnarray}
  \DIFadd{|\psi_{\kappa }^{\lambda }\rangle
  }&\DIFadd{=}& \DIFadd{2^{-3} \sum_{\alpha,\gamma \in \mathbb{F}_{2^{3}}}
  \Phi_{\alpha,\lambda \alpha} \;
  %DIF > (-1)^{\tr(\alpha (\kappa + \gamma))}
  \chi\left( \alpha(\kappa+\gamma) \right) 
  |\gamma \rangle, }\\ \DIFadd{[6pt]}
  \DIFadd{|\tilde{\psi}_{\kappa }\rangle
  }&\DIFadd{=}& \DIFadd{2^{-3 / 2} \sum_{\gamma \in \mathbb{F}_{2^{3}}}
  %DIF > (-1)^{\tr(\kappa \gamma)}
  \chi\left( \kappa\gamma \right) 
  |\gamma \rangle,
}\end{eqnarray}

\DIFadd{and the phases $\Phi_{\alpha,\beta}$ are defined in (\ref{ray dps}).
...
%DIF >  Please provide either the correct journal abbreviation (e.g. according to the
%DIF >  "List of Title Word Abbreviations" http://www.issn.org/services/online-services/access-to-the-ltwa/) or the full name of the journal.
