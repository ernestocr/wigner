...
\begin{document}
  ...
  xy$. El teorema \ref{thm:kanat_even} nos brinda los
  siguientes vectores:
  \[
    \ket{b_{m,v}}
    = \frac{1}{\sqrt{d}}
  ...
  \begin{figure}[ht]
    ...
    \caption{$V = \GF(2^{5})$, spread Desarguesiano.}
  ...
  Notemos que los rayos formados son distintos a los del
  spread Desarguesiano.
  \begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{example-image-a}
    \caption{$V = \GF(2^{5})$, spread de Kantor.}
  ...
  \end{equation}
  Con ésto ya tenemos todo listo para calcular de manera
  directa al conjunto completo de MUBs utilizando el teorema
  \ref{thm:kanat_even}. Dicho teorema nos dice que los
  eigenvectores de los operadores $D(u,u \circ m)$ son:
  \begin{equation}
    \ket{b_{m,v}}
    = \frac{1}{\sqrt{d}} 
    \sum_{w \in F}^{} \omega^{\tr\left( \hat w (\hat w \circ
    m) + 2 \hat w \hat v\right) } \ket{e_w}.
  \end{equation}
  ...
  y () respectivamente como el estado correspondiente al
  rayo. Los estados posteriores de cada estría quedan
  determinados por los operadores de traslación
  correspondientes.

  Retomamos los ejemplos () y () para los cuales hemos
  construidos dos conjuntos de MUBs no equivalentes. El
  primero conjunto coincide con las construcciones de
  Wootters.
