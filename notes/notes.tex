...
\begin{document}
  ...
  %Specifically, the two equations are $x^2-y^2=0$ and
  %$x^{p^s+1}-y^{p^s+1}=0$ where $p$ is an odd prime and the
  %polynomials are over the finite field of order $p^n$. The
  %parameter $s$ is relatively prime to $n$ and $1 \leq s < n
  %/ 2$ but I think it doesn't matter. For the simple case
  %$p=3$, $n=3$ and $s=1$ it was easy to prove the equality
  %of the solutions since $x^{4}-y^{4} = (x^2+y^2)(x^2-y^2) =
  %0$ and $x^2+y^2=0$ does not have a solution in the finite
  %field of $3^3$ elements, but I think this argument doesn't
  %work in general. Any tips or suggestions?

  %Well you just want the solution sets of $x^2=1$ and
  %$x^{p^s+1}=1$ to be equal, which means that there are only
  %two solutions to $x^{p^s+1}=1$. Indeed, let $g$ be a
  %generator of the cyclic group $F^*$, the multiplicative
  %group of the finite field F. The order of $g^{p^s+1}$ is
  %$(p^n-1) / \gcd(p^n-1,p^s+1)$, so the kernel of the map $x
  %\mapsto x^{p^s+1}$ has size $\gcd(p^n-1,p^s+1)$, which
  %must be 2.  Conversely, if this is 2, we get the desired
  %solutions since there's a unique cyclic subgroup of order
  %2.

  %You just need to see when $2 = \gcd(2,p^n-1) =
  %\gcd(p^s+1,p^n-1)$.  Suppose that $d | p^s+1$, $p^n-1$.
  %Then $d | p^{2s}-1$, $p^n-1$. By the Euclidean algorithm
  %on the exponents, $d|p^{gcd(2s,n)}-1$ and so $d|p^2-1$. If
  %s is even, then we conclude that $p^s=1 \mod d$, so $d|2$.
  %If s is odd, then $p^s=p \mod d$, so $d|p+1$. If $n$ is
  %odd, then $p^n=p \mod d$, and so $d|2$ again. If $n$ is
  %even, then $p+1|p^s+1,p^n-1$, and so the condition breaks
  %precisely when $n$ is even, $s$ is odd.
